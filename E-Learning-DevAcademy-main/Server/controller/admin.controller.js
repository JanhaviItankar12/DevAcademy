...
export const adminDashboard = async (req, res) => {
  ...
  if (!['week', 'month', 'year'].includes(range)) {
    return res.status(400).json({ error: 'Invalid range parameter' });
        ...
        const revenueGrowth = ((revenueThisMonth[0]?.totalRevenue || 0) - (revenueLastMonth[0]?.totalRevenue || 0)) / ((revenueLastMonth[0]?.totalRevenue || 1)) * 100;
        const studentGrowth = ((studentsThisMonth) - (studentsLastMonth)) / (studentsLastMonth || 1) * 100;
